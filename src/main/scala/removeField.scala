...
object removeField {
  ...
  trait RemoveField[T] {
    type Out <: HList
    def removeField(t: T, field: Witness.Aux[Symbol]): Out
  }
  ...
  object RemoveField {
    ...
    implicit def generic[F, G, R <: HList](implicit gen: LabelledGeneric.Aux[F, G], sg: Lazy[RemoveField.Aux[G, R]]): RemoveField.Aux[F, R] =
      new RemoveField[F] {
        type Out = R
        def removeField(f: F, field: Witness.Aux[Symbol]) = sg.value.removeField(gen.to(f), field)
      }

    implicit def product: RemoveField.Aux[HNil, HNil] =
      new RemoveField[HNil] {
        type Out = HNil
        def removeField(p: HNil, field: Witness.Aux[Symbol]) = HNil
      }
    ...
    implicit def product[K <: Symbol, V, T <: HList, RV <: HList, RT <: HList, R <: HList](implicit
      ...
      prepend: Prepend.Aux[RV, RT, R]): RemoveField.Aux[FieldType[K, V] :: T, R] =
      new RemoveField[FieldType[K, V] :: T] {
        type Out = R
        def removeField(p: FieldType[K, V] :: T, field: Witness.Aux[Symbol]) = {
          val currentField: Symbol = selector(p)
          val fieldToRemove: Symbol = field.value
          if (currentField == fieldToRemove) sv.value.removeField(p.head, field)
    ...
    implicit def cnil: RemoveField.Aux[CNil, HNil] =
      new RemoveField[CNil] {
        type Out = HNil
        def removeField(p: CNil, field: Witness.Aux[Symbol]) = HNil
      }
    ...
    implicit def coproduct[K <: Symbol, V, T <: Coproduct, RV <: HList, RT <: HList, R <: HList](implicit
      ...
      new RemoveField[FieldType[K, V] :+: T] {
        type Out = R
        def removeField(c: FieldType[K, V] :+: T, field: Witness.Aux[Symbol]): HList =
          c match {
  ...
  implicit class RemoveFieldOps[T](x: T)(implicit removeFieldT: RemoveField[T]) {
    def removeField(field: Witness.Aux[Symbol]): HList = removeFieldT.removeField(x, field)
  }
